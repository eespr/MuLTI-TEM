...
% ! THIS CODE INPUTS THREE BASE FREQUENCIES !  %
...
load('2D_input_data_III.mat'); % load 2D received voltages
...
% This versions inputs THREE BASE FREQUENCIES
%      30 Hz     %
REFTYM1 = 8.325;
OFFTYM1 = 8.325;
TXON1   = [0.0, 0.46 7.865, 8.325];
TOPN1   = [0.005800, 0.007800, 0.010400, 0.013700, 0.018000, ...
...
TCLS1   = [0.007800, 0.010400, 0.013700, 0.018000, ...
          0.023500, 0.030500, 0.03900, 0.04980000, 0.0628000, ...
          0.077800, 0.094100, 0.11530, 0.142800, 0.176600, ...
          0.220300, 0.276600, 0.347800, 0.439000, 0.555300, ...
          0.702800, 0.891500, 1.13100, 1.438000, 1.829000, ...
          2.328000, 2.96400, 3.776000, 4.81300, 6.13400, 7.81900];
Centre1 = (TCLS1-TOPN1)./2 + TOPN1;

%      7.5 Hz     %
REFTYM2 = 33.25;
OFFTYM2 = 33.25;
TXON2   = [0.0, 0.46 32.79, 33.25];
TOPN2   = [0.032000, 0.040000, 0.050500, 0.063500, 0.081000, ...
...
TCLS2   = [0.040000, 0.050500, 0.063500, 0.081000, ...
          0.10300, 0.13100, 0.16500, 0.208000, 0.260000, ...
          0.320000, 0.385000, 0.470000, 0.580000, 0.715000, ...
          0.890000, 1.115000, 1.40000, 1.765000, 2.23000, ...
          2.8200000, 3.575000, 4.53500, 5.760000, 7.325000, ...
          9.320000, 11.86500, 15.11500, 19.2600, 24.54500, 31.2850];
Centre2 = (TCLS2-TOPN2)./2 + TOPN2;

%      3 Hz     %
REFTYM3 = 83.25;
OFFTYM3 = 83.25;
TXON3   = [0.0, 0.46 82.79, 83.25];
TOPN3   = [0.080000, 0.100000, 0.126300, 0.158800, 0.202500, ...
...
TCLS3   = [0.100000, 0.126300, 0.158800, 0.202500, ...
          0.257500, 0.327500, 0.412500, 0.520000, 0.650000, ...
          0.800000, 0.963000, 1.175000, 1.450000, 1.788000, ...
          2.225000, 2.790000, 3.500000, 4.413000, 5.575000, ...
          7.050000, 8.940000, 11.33800, 14.40000, 18.31000, ...
          23.30000, 29.66300, 37.80000, 48.15000, 61.36000, 78.2000];
Centre3 = (TCLS3-TOPN3)./2 + TOPN3;
...
CI_density_FM2_2D = cell(1,no_curves);
CI_density_FMN2_2D = cell(1,no_curves);
CI_density_FM3_2D = cell(1,no_curves);
CI_density_FMN3_2D = cell(1,no_curves);
...
FM_edge2_2D =cell(1,no_curves);
time_edge2_2D = cell(1,no_curves);
FM_edge3_2D =cell(1,no_curves);
time_edge3_2D = cell(1,no_curves);
salt_edge_2D = cell(1,no_curves);
poro_edge_2D = cell(1,no_curves);
y_2D = cell(1,no_curves);
time_x_2D = cell(1,no_curves);
yVolts2_2D = cell(1,no_curves);
time_x2_2D = cell(1,no_curves);
yVolts3_2D = cell(1,no_curves);
time_x3_2D = cell(1,no_curves);
...
parfor c = 1:no_curves
	...
	% 7.5 Hz
	data2 = data_2D{2,c}(:,2);% observed data, received voltage in nanoVolts
    timegate_centre2 = data_2D{2,c}(:,1);% define time gate centre in ms
	...
	% 3 Hz
	data3 = data_2D{3,c}(:,2);% observed data, received voltage in nanoVolts
    timegate_centre3 = data_2D{3,c}(:,1);% define time gate centre in ms
	weighting3 = data3.*0.1; %determine weighting to be applied in inversion in nV
    nd3 =numel(data3); % nd is the number of data points
...
time_x2=linspace(log10(timegate_centre2(1,1)), log10(timegate_centre2(end,1)), dis);%discretize the forward model (time-axis) 
time_x3=linspace(log10(timegate_centre3(1,1)), log10(timegate_centre3(end,1)), dis);%discretize the forward model (time-axis) 
yVolts = linspace(log10(min(data-weighting)), log10(max(data+weighting)), dis); %discretize the forward model (recieved voltage axis)
yVolts2 = linspace(log10(min(data2-weighting2)), log10(max(data2+weighting2)), dis); %discretize the forward model (recieved voltage axis)
yVolts3 = linspace(log10(min(data3-weighting3)), log10(max(data3+weighting3)), dis); %discretize the forward model (recieved voltage axis)
...
CI_density_FM2 = zeros((dis-1),length(time_x2)-1);
CI_density_FM3 = zeros((dis-1),length(time_x3)-1);
...
if priors_OK == 1
    ...
    if running_mode == 1
	% base frequency 1 (30 Hz)
        forward_model = LEROI_TEM(NCHNL, REFTYM1, OFFTYM1, TXON1, TXAMP, TOPN1, TCLS1, SXE, SXN, RXE, RXN, RXZ, npt+num_layers, 0, npt + num_layers, Res, linspace(1,(npt+num_layers),(npt+num_layers)), thickness);
        forward_model = round(forward_model,6,'significant');       
		%%%%%%%%%% computing the misfit %%%%%%%%%%%
		misfit = NaN(length(data),1);
        for i = 1:length(data) % time samples, this should match the time samples.
            misfit(i,1) = data(i,1) - forward_model(i,1);
        end %end multimodal misfit   
		like = nansum( (misfit).^2 ./(2 * weighting.^2) );
	% base frequency 2 (7.5 Hz)
		forward_model2 = LEROI_TEM(NCHNL, REFTYM2, OFFTYM2, TXON2, TXAMP, TOPN2, TCLS2, SXE, SXN, RXE, RXN, RXZ, npt+num_layers, 0, npt + num_layers, Res, linspace(1,(npt+num_layers),(npt+num_layers)), thickness);
        forward_model2 = round(forward_model2,6,'significant');       
		%%%%%%%%%% computing the misfit %%%%%%%%%%%
		misfit2 = NaN(length(data2),1);
        for i = 1:length(data2) % time samples, this should match the time samples.
            misfit2(i,1) = data2(i,1) - forward_model2(i,1);
        end %end multimodal misfit   
		like2 = nansum( (misfit2).^2 ./(2 * weighting2.^2) );
	% base frequency 3 (3 Hz)
		forward_model3 = LEROI_TEM(NCHNL, REFTYM3, OFFTYM3, TXON3, TXAMP, TOPN3, TCLS3, SXE, SXN, RXE, RXN, RXZ, npt+num_layers, 0, npt + num_layers, Res, linspace(1,(npt+num_layers),(npt+num_layers)), thickness);
        forward_model3 = round(forward_model3,6,'significant');       
		%%%%%%%%%% computing the misfit %%%%%%%%%%%
		misfit3 = NaN(length(data3),1);
        for i = 1:length(data3) % time samples, this should match the time samples.
            misfit3(i,1) = data3(i,1) - forward_model3(i,1);
        end %end multimodal misfit   
		like3 = nansum( (misfit3).^2 ./(2 * weighting3.^2) );
    else
    like = 1;
	like2 = 1;
	like3 = 1;
...
% find which layer the nuclei will move to:
           ...
           Res = zeros(length(R),7);
            ...
            if running_mode == 1
                if like == 0 
					...
					forward_model_p2 = nan(length(data2),1);
					forward_model_p3 = nan(length(data3),1);
                else
                    forward_model_p = forward_model;
					forward_model_p2 = forward_model2;
					forward_model_p3 = forward_model3;
                end
			% 30 Hz
                forward_model = LEROI_TEM(NCHNL, REFTYM1, OFFTYM1, TXON1, TXAMP, TOPN1, TCLS1, SXE, SXN, RXE, RXN, RXZ, npt_prop+num_layers, 0, npt_prop + num_layers, Res, linspace(1,(npt_prop+num_layers),(npt_prop+num_layers)), thickness);
                forward_model = round(forward_model,6,'significant'); 
                %%%%%%%%%% computing the misfit %%%%%%%%%%%
                misfit = NaN(length(data),1);
                for i = 1:length(data) % time samples, this should match the time samples.
			...
			% 7.5 Hz
                forward_model2 = LEROI_TEM(NCHNL, REFTYM2, OFFTYM2, TXON2, TXAMP, TOPN2, TCLS2, SXE, SXN, RXE, RXN, RXZ, npt_prop+num_layers, 0, npt_prop + num_layers, Res, linspace(1,(npt_prop+num_layers),(npt_prop+num_layers)), thickness);
                forward_model2 = round(forward_model2,6,'significant'); 
                %%%%%%%%%% computing the misfit %%%%%%%%%%%
                misfit2 = NaN(length(data2),1);
                for i = 1:length(data2) % time samples, this should match the time samples.
                ...
                like_prop2 = nansum( (misfit2).^2 ./(2 * weighting2.^2) );
			% 3 Hz
                forward_model3 = LEROI_TEM(NCHNL, REFTYM3, OFFTYM3, TXON3, TXAMP, TOPN3, TCLS3, SXE, SXN, RXE, RXN, RXZ, npt_prop+num_layers, 0, npt_prop + num_layers, Res, linspace(1,(npt_prop+num_layers),(npt_prop+num_layers)), thickness);
                forward_model3 = round(forward_model3,6,'significant'); 
                %%%%%%%%%% computing the misfit %%%%%%%%%%%
                misfit3 = NaN(length(data3),1);
                for i = 1:length(data3) % time samples, this should match the time samples.
                ...
                like_prop3 = nansum( (misfit3).^2 ./(2 * weighting3.^2) );
            else
                like_prop = 1;
				like_prop2 = 1;
				like_prop3 = 1;
 ...
 % (for the result is always 0).
    ...
    if (birth==1)
		rd = rand;	
        if (rd<((1/(Prod_delta_prior*prob))*exp(log(out)-like_prop+like)))...
		&& (rd<((1/(Prod_delta_prior*prob))*exp(log(out)-like_prop2+like2)))...
		&& (rd<((1/(Prod_delta_prior*prob))*exp(log(out)-like_prop3+like3)))
            accept=1;
            if (s>burn_in)
                AB=AB+1;
            end
    ...
    elseif (death==1)
        rd = rand;
        if (rd<(Prod_delta_prior*prob*exp(log(out)-like_prop+like)))...
		&& (rd<(Prod_delta_prior*prob*exp(log(out)-like_prop2+like2)))...
		&& (rd<(Prod_delta_prior*prob*exp(log(out)-like_prop3+like3)))
            accept=1;
            if (s>burn_in)
                AD=AD+1;
            end
    ...
    elseif (move == 1) % NO JUMP, i.e no change in dimension
        rd = rand;
        if (rd<(move_prob * exp(log(out)-like_prop+like)))...
		&& (rd<(move_prob * exp(log(out)-like_prop2+like2)))...
		&& (rd<(move_prob * exp(log(out)-like_prop3+like3)))
            accept=1;
            if (s>burn_in)
            AP=AP+1;       
            end %if (s>burn_in)
     ...
     % CHANGE R
		...
		rd = rand;
        if  (rd<exp(log(out)-like_prop+like))...
		&& (rd<exp(log(out)-like_prop2+like2))...
		&& (rd<exp(log(out)-like_prop3+like3))
        accept=1;
            if (s>burn_in)
            AcV=AcV+1;       
            end %if (s>burn_in)      
    ...
    % 30 Hz
        ...
        if neg == 1 % no negative TEM responses counted in the PDF as they are not in the plots scale, only showing positive TEM responses
            ...
            [FM]=histcounts2(fmp_interp,time_x,yVolts,time_x); % TEM response 30 Hz matrix
        end
	% 7.5 Hz
		neg = any(forward_model_p2<0);
        if neg == 1 % no negative TEM responses counted in the PDF as they are not in the plots scale, only showing positive TEM responses
            [FM2] = zeros((dis-1),(dis-1));
        else
            fmp_interp2 = interp1(log10(timegate_centre2),log10(forward_model_p2),time_x2);
            [FM2]=histcounts2(fmp_interp2,time_x2,yVolts2,time_x2); % TEM response 7.5 Hz matrix
        end
	% 3 Hz
		neg = any(forward_model_p3<0);
        if neg == 1 % no negative TEM responses counted in the PDF as they are not in the plots scale, only showing positive TEM responses
            [FM3] = zeros((dis-1),(dis-1));
        else
            fmp_interp3 = interp1(log10(timegate_centre3),log10(forward_model_p3),time_x3);
            [FM3]=histcounts2(fmp_interp3,time_x3,yVolts3,time_x3); % TEM response 3 Hz matrix
        end
		...
		FM_edge2=yVolts2(1:(dis-1));
        time_edge2=time_x2(1:(dis-1));
		FM_edge3=yVolts3(1:(dis-1));
        time_edge3=time_x3(1:(dis-1));
    ...
    if (s>burn_in)
        if (mod(s,thin)==0)
            ...
            if running_mode == 1
				CI_density_FM = CI_density_FM + FM; % 30 hz TEM ensemble
				CI_density_FM2 = CI_density_FM2 + FM2; % 7.5 hz TEM ensemble
				CI_density_FM3 = CI_density_FM3 + FM3; % 3 hz TEM ensemble
            end
            if Archie > 0
				if Archie == 1
    ...
    if priors_OK ==1 && (like<like_best) 
        ...
        if running_mode ==1
			...
			forward_model_best2 = forward_model2;
			forward_model_best3 = forward_model3;
        end
    end   
...
if running_mode == 1
	...
	CI_density_FM2_2D{1,c} = CI_density_FM2;
	CI_density_FM3_2D{1,c} = CI_density_FM3;
end
...
time_x2_2D{1,c} = time_x2;
time_x3_2D{1,c} = time_x3;
yVolts_2D{1,c} = yVolts;
yVolts2_2D{1,c} = yVolts2;
yVolts3_2D{1,c} = yVolts3;
FM_edge_2D{1,c} =FM_edge;
FM_edge2_2D{1,c} =FM_edge2;
FM_edge3_2D{1,c} =FM_edge3;
time_edge_2D{1,c} = time_edge;
time_edge2_2D{1,c} = time_edge2;
time_edge3_2D{1,c} = time_edge3;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 for c = 1:no_curves
    ...
    if running_mode ==1
		...
		sizeCI = size(CI_density_FM2_2D{1,c});
        len = sizeCI(1,1);
        wid = sizeCI(1,2);
        CI_density_FMN2_2D{1,c} = zeros(len,wid);
        for i = 1:len
            m = sum(CI_density_FM2_2D{1,c}(i,:));
            for j = 1:wid
                CI_density_FMN2_2D{1,c}(i,j) = CI_density_FM2_2D{1,c}(i,j)/m;
            end
        end
		sizeCI = size(CI_density_FM3_2D{1,c});
        len = sizeCI(1,1);
        wid = sizeCI(1,2);
        CI_density_FMN3_2D{1,c} = zeros(len,wid);
        for i = 1:len
            m = sum(CI_density_FM3_2D{1,c}(i,:));
            for j = 1:wid
                CI_density_FMN3_2D{1,c}(i,j) = CI_density_FM3_2D{1,c}(i,j)/m;
            end
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
