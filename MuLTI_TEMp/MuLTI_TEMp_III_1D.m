...
load('1D_input_III.mat'); % load 2D received voltages

% data = 30 Hz received voltage in nanoVolts
...
% data3 = 3 Hz received voltage in nanoVolts
timegate3 = data3(:,1:2);% define start and end times of the time gates in ms, this goes into the forward model calculation
timegate_centre3 = data3(:,3);% define time gate centre in ms
data3 = data3(:,4); % set observed data, received voltage in nanoVolts
...
if priors_OK == 1
	...
	% base frequency 2 (7.5 Hz)
        ...
        for i = 1:length(timegate2) % time samples, this should match the time samples.
	...
	% base frequency 3 (3 Hz)
        ...
        for i = 1:length(timegate3) % time samples, this should match the time samples.
...
if Archie > 0
    ...
    ylabel('Depth (m)','FontSize',12);
    xlabel('Salinity (log10 psu)','FontSize',12);
    set(gca,'XMinorTick','on') 
    set(gca,'Ydir','reverse')
    ylim([700 1000])
    colormap(cm_jet)
...
%%%%%%%%%%%%%%%%%%% 30 hz
subplot(2,3,[1 4])
imagesc(time_edge,FM_edge,CI_density_FM) %pdf plot
hold on;
plot( log10(timegate_centre), log10(data), '*g','markersize',8);
hold on;
plot( log10(timegate_centre), log10(data+weighting),'--g','markersize',8);
hold on;
plot( log10(timegate_centre), log10(data-weighting), '--g','markersize',8);
% Create ylabel
...
%%%%%%%%%%%%%%%%%%% 7.5 hz
subplot(2,3,[2 5])
imagesc(time_edge2,FM_edge2,CI_density_FM2) %pdf plot
hold on;
plot( log10(timegate_centre2), log10(data2), '*g','markersize',8);
hold on;
plot( log10(timegate_centre2), log10(data2+weighting2),'--g','markersize',8);
hold on;
plot( log10(timegate_centre2), log10(data2-weighting2), '--g','markersize',8);
% Create ylabel
ylabel('Received voltage (nV)');
...
%%%%%%%%%%%%%%%%%% 3 hz
subplot(2,3,[3 6])
imagesc(time_edge3,FM_edge3,CI_density_FM3) %pdf plot
hold on;
plot( log10(timegate_centre3), log10(data3), '*g','markersize',8);
hold on;
plot( log10(timegate_centre3), log10(data3+weighting3),'--g','markersize',8);
hold on;
plot( log10(timegate_centre3), log10(data3-weighting3), '--g','markersize',8);
% Create ylabel
ylabel('Received voltage (nV)');
